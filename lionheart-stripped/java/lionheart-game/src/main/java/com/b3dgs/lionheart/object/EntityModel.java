...
public final class EntityModel extends EntityModelHelper implements XmlLoader, XmlSaver, Editable<ModelConfig>,
                               RoutineUpdate, Recyclable, Snapshotable
{
    ...
    // network strip: networkedDevice + deviceNetwork removed
                       ...
                       StateHandler state)
    {
        ...
        // network strip: state.addListener(syncState) removed
    }
    ...
    // network strip: syncState + giveClientControl + onConnected removed (single-player only)
    ...
    {
        // network strip: deviceNetwork update removed
        jump.update(extrp);
    ...
    // network strip: prepare() networkedDevice probe + onReceived removed
